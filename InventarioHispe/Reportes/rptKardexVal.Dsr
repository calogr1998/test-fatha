VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} rptKardexVal 
   Caption         =   "Logistica_Suma - rptKardexVal (ActiveReport)"
   ClientHeight    =   8040
   ClientLeft      =   1185
   ClientTop       =   1845
   ClientWidth     =   12615
   ...
   _ExtentX        =   22251
   _ExtentY        =   14182
   SectionData     =   "rptKardexVal.dsx":0000
End
Attribute VB_Name = "rptKardexVal"
...
'    IMPRIMIR_VALES '''(rpt, Link, strSQL)
    ...
    With rptValeIngreso
        .TipoVale = objSqlAyudaVale.TipoVale
        .CodAlmacen = objSqlAyudaVale.CodigoAlmacen
        .NumeroVale = objSqlAyudaVale.NumeroVale
        
        'ModMilano.abrirCnDBMilano
        
        .fldCategoria.Text = ModUtilitario.ObtenerCampoV2(cnBdStudioModa, "CT.NOMBRE", "ORDENPRODUCCION AS OP LEFT JOIN CATEGORIATIPO AS CT ON CT.IDCATEGORIATIPO = OP.IDCATEGORIATIPO", "OP.IDORDENPRODUCCION", Val(objSqlAyudaVale.OrdenTrabajo), "N")
        .fldOP.Text = ModUtilitario.ObtenerCampoV2(cnBdStudioModa, "OP", "ORDENPRODUCCION", "IDORDENPRODUCCION", Val(objSqlAyudaVale.OrdenTrabajo), "N")
...
Private Sub GroupHeader1_BeforePrint()
    ...
    'rptKardexVal.fldcant_ant = Format(wcant, "###,###,##0.00")
    'rptKardexVal.fldcant_ant.OutputFormat = "#,##0.00;(#,##0.00)"
