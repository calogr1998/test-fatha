VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} rptOrdenPendienteAtencion 
   Caption         =   "Logistica - rptOrdenPendienteAtencion (ActiveReport)"
   ClientHeight    =   8010
   ClientLeft      =   960
   ClientTop       =   2955
   ClientWidth     =   12570
   Icon            =   "rptOrdenPendienteAtencion.dsx":0000
   WindowState     =   2  'Maximized
   _ExtentX        =   22172
   _ExtentY        =   14129
   SectionData     =   "rptOrdenPendienteAtencion.dsx":058A
End
Attribute VB_Name = "rptOrdenPendienteAtencion"
...
Private Sub ActiveReport_ReportStart()
    ...
    Me.Caption = App.ProductName & " - Atención de O/C's por Proveedor: " & ModUtilitario.ObtenerCampoV2(cnn_dbbancos, "TRIM(F2NEWRUC & ' - ' & F2NOMPROV) AS PROVEEDOR", "EF2PROVEEDORES", "F2CODPROV", strCodProveedor, "T")
...
Private Sub imprimirOrdenMovimiento()
    ...
    With objAyudaOrden
        ...
        'strSQL = .devuelveSQLOrdenPendienteAtencionPorProveedor(bolMostrarNroRequerimiento, bolSoloProductoConSaldo)
        
        strSQL = .devuelveSQLOrdenPendienteAtencionPorProveedorV2(False, bolSoloProductoConSaldo, bolMostrarNroRequerimiento)
    End With
    
    lblRequerimiento.Visible = bolMostrarNroRequerimiento
    fldRequerimient.Visible = bolMostrarNroRequerimiento
    
    If Not bolMostrarNroRequerimiento Then
        lblOC.Width = lblOC.Width + lblRequerimiento.Width
        fldOC.Width = fldOC.Width + fldRequerimient.Width
    End If
