...
Dim SQL             As String
Dim DBTable1        As String
Dim wvale           As String
Dim basetemporal    As String
        ...
        If wtipoayuda = "G" Then
            cfecha = Format("" & abodesde.Value, "dd/mm/yyyy")
               ...
               If IsDate(cfecha) Then
                     ...
                     If Len(wusuario) > 0 Then
                         SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
                         "where F4ESTNUL = 'N' and F4ESTFAC = 'N' and F4TIPODOCU = '09' and F2CODCLI= '" & Trim(wusuario) & "' and F4FECEMI >= CVDATE('" & cfecha & "') and F4FECEMI<=CVDATE('" & cfecha & "')"
               ...
               Else
                    SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
                         "where F4ESTNUL = 'N' and F4ESTFAC = 'N' and F4TIPODOCU = '09' and F2CODCLI= '" & Trim(wusuario) & "'"
                    TxtSerie.SetFocus
               End If
         Else
...
Private Sub txtserie_Change()
    ...
    If wtipoayuda = "G" Then
        ...
        If Len(cserie) > 0 Then
            If Len(wusuario) > 0 Then
                 SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
                 "where F4ESTNUL = 'N' and F4ESTFAC = 'N' and F4TIPODOCU = '09' and F2CODCLI= '" & Trim(wusuario) & "' and f4serdoc >='" & cserie & "' and f4serdoc<='" & cserie & "z" & "'"
             Else
                 SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
    ...
    If wtipoayuda = "G" Then
        cdocumento = Trim(txtdocumento)
           ...
           If Len(txtdocumento) > 0 Then
                 If Len(wusuario) > 0 Then
                     SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
                     "where F4ESTNUL = 'N' and F4ESTFAC = 'N' and F4TIPODOCU = '09' and F2CODCLI= '" & Trim(wusuario) & "' and F4NUMDOC >='" & cdocumento & "' and F4NUMDOC<='" & cdocumento & "z" & "'"
                 Else
                     SQL = "SELECT F4SERDOC,F4NUMDOC,F4FECEMI,F2NOMCLI FROM TBVENTA_CAB " & _
...
Dim CON     As String
    ...
    basetemporal = wusuario & "Centros" & Format(Time, "hh_mm_ss") & ".MDB"
    CREATEDATABASE_N wrutatemp & "\", CStr(basetemporal)
    CON = "Provider=Microsoft.JET.OLEDB.4.0; Data Source=" & wrutatemp & "\" & basetemporal & "; Persist Security Info=False"
    temp.Open CON
