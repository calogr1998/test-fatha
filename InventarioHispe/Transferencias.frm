...
Begin VB.Form Transferencias 
   ...
   Begin ActiveToolBars.SSActiveToolBars SSActiveToolBars1 
      ...
      ToolsCount      =   7
      PersonalizedMenus=   0
      Tools           =   "Transferencias.frx":0000
      ToolBars        =   "Transferencias.frx":58B6
   End
   Begin Threed.SSPanel txtfecha 
      Height          =   6120
      Left            =   45
      TabIndex        =   0
      Top             =   45
      ...
      Begin DXDBGRIDLibCtl.dxDBGrid dxDBGrid1 
         Height          =   4065
         Left            =   45
         OleObjectBlob   =   "Transferencias.frx":5A14
         TabIndex        =   16
         Top             =   1980
         Width           =   10275
      End
...
Dim sw_ayuda As Boolean, sw_ayudaO  As Boolean
...
Private Sub ACTUALIZAR()
    ...
    GRABA_REGISTRO_logistica amovs_vale(), "EF2ALMACENES", "M", 0, cnn_dbbancos, "F2CODALM ='" & Trim(Txtcodalm.Text) & "'"
...
Dim TOTA As Double
    ...
    If ctipo = "A" Then '---Nuevo
        '-----Graba Cabecera
        GRABA_REGISTRO_logistica amovs_cab(), "IF4VALES", "A", 14, cnn_dbbancos, ""
        If sw_GRABA_REGISTRO_logistica = True Then
            '------- GRABA DETALLE
            GRABA_REGISTRO_logistica_DET amovs_det(), "IF3VALES", "A", 8, cnn_dbbancos, "", Values(), nfila - 1, "111111111111", "", ""
        End If
    Else    '----------Modificacion
        '------- GRABA CABECERA
        GRABA_REGISTRO_logistica amovs_cab(), "IF4VALES", "M", 14, cnn_dbbancos, "F4NUMVAL = '" & Txtnumvalo.Caption & "' AND F2CODALM = '" & Txtcodalm.Text & "'"
        '------- GRABA DETALLE
        sql = ("DELETE * FROM IF3VALES WHERE F4NUMVAL = '" & Txtnumvalo.Caption & "' AND F2CODALM = '" & Txtcodalm.Text & "'")
        cnn_dbbancos.Execute sql
        AlmacenaQuery_sql sql, cnn_dbbancos
        Actualiza_Log sql, cnn_dbbancos.ConnectionString
        GRABA_REGISTRO_logistica_DET amovs_det(), "IF3VALES", "A", 8, cnn_dbbancos, "F4NUMVAL  = '" & Txtnumvalo.Caption & "' AND F2CODALM = '" & Txtcodalm.Text & "'", Values(), nfila - 1, "111111111111", "", ""
    End If
    ...
    If wIndEnvia = "*" Then 'cnn_dbEnvia
        ...
        Actualiza_Log sql, cnn_dbbancos.ConnectionString
        GRABA_REGISTRO_logistica amovs_cab(), "IF4VALES", "A", 14, cnn_dbEnvia, ""
        If sw_GRABA_REGISTRO_logistica = True Then
            '------- GRABA DETALLE
            GRABA_REGISTRO_logistica_DET amovs_det(), "IF3VALES", "A", 8, cnn_dbEnvia, "", Values(), nfila - 1, "111111111111", "", ""
        End If
    End If
    ''''''''
End Sub
...
Private Sub ACTUALIZAR1()
    ...
    GRABA_REGISTRO_logistica amovs_vale(), "EF2ALMACENES", "M", 0, cnn_dbbancos, "F2CODALM ='" & Trim(Txtcodpar.Text) & "'"
...
Dim TOTA As Double
    ...
    Else    '----------Modificacion
        '------- GRABA CABECERA
        GRABA_REGISTRO_logistica amovs_cab(), "IF4VALES", "M", 14, cnn_dbbancos, "F4NUMVAL = '" & Txtnumvald.Caption & "' AND F2CODALM = '" & Txtcodpar.Text & "'"
        '------- GRABA DETALLE
        
        sql = ("DELETE * FROM IF3VALES WHERE F4NUMVAL = '" & Txtnumvald.Caption & "' AND F2CODALM = '" & Txtcodpar.Text & "'")
        cnn_dbbancos.Execute sql
        AlmacenaQuery_sql sql, cnn_dbbancos
        Actualiza_Log sql, cnn_dbbancos.ConnectionString
        GRABA_REGISTRO_logistica_DET amovs_det(), "IF3VALES", "A", 8, cnn_dbbancos, "F4NUMVAL  = '" & Txtnumvald.Caption & "' AND F2CODALM = '" & Txtcodpar.Text & "'", Values(), nfila - 1, "111111111111", "", ""
    End If
...
Private Sub dxDBGrid1_OnKeyDown(KeyCode As Integer, ByVal Shift As Long)
    If KeyCode = 115 Or KeyCode = 46 Then
        If MsgBox("¿Desea Eliminar el ítem selecionado?", vbQuestion + vbYesNo, "Atención") = vbYes Then
            sw_detalle = True
            If dxDBGrid1.Dataset.RecNo = 1 Then
                dxDBGrid1.Dataset.Delete
                AdicionaItem
