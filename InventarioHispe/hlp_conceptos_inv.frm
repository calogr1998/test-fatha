...
Begin VB.Form hlp_conceptos_inv 
   Caption         =   "Conceptos de Almacén"
   ClientHeight    =   3705
   ClientLeft      =   1650
   ClientTop       =   1995
   ClientWidth     =   6660
   ...
   Begin Threed.SSPanel SSPanel1 
      ...
      Begin TrueOleDBGrid70.TDBGrid datagrid 
         Bindings        =   "hlp_conceptos_inv.frx":0000
         Height          =   2670
         Left            =   90
         TabIndex        =   1
         Top             =   90
         ...
         Splits(0)._ColumnProps(11)=   "Column(1)._ColStyle=74256"
   ...
   Begin ActiveToolBars.SSActiveToolBars SSActiveToolBars1 
      ...
      ToolBarsCount   =   1
      ToolsCount      =   1
      Tools           =   "hlp_conceptos_inv.frx":0018
      ToolBars        =   "hlp_conceptos_inv.frx":008C
   End
   ...
   End
End
Attribute VB_Name = "hlp_conceptos_inv"
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Case vbKeyF4
        If datagrid.Enabled Then datagrid.SetFocus
    Case vbKeyF5:
        CambiarFiltro
    End Select
End Sub
...
Private Sub TxtFiltro_Change(Index As Integer)
    ...
    '
    datagrid.Columns(Index).FilterText = cValFil
    DataGrid_FilterChange
    '
...
Private Sub MostrarGrid()
    ...
    If wtipmov = "I" Then cValSql = "Select F1CODORI,F1NOMORI FROM SF1ORIGENES WHERE F1TIPMOV='I' ORDER BY F1CODORI"
    If wtipmov = "S" Then cValSql = "Select F1CODORI,F1NOMORI FROM SF1ORIGENES WHERE F1TIPMOV='S' ORDER BY F1CODORI"
    
    If Len(Trim(wtipmov)) = 0 Then
        cValSql = "Select F1CODORI,F1NOMORI FROM SF1ORIGENES ORDER BY F1CODORI"
    End If
    ...
    '
    datagrid.Columns(0).DataField = "F1CODORI"
    datagrid.Columns(1).DataField = "F1NOMORI"
    Set datagrid.DataSource = rstAyuda
    '
    Exit Sub
...
Private Sub Elegir()
    wconcepto = datagrid.Columns(0)
    wnomconcepto = datagrid.Columns(1)
    Unload Me
End Sub
...
Private Function Filtrar() As String
    ...
    cValFil = ""
    '
    For Each oColLis In datagrid.Columns
