...
'    Cad = "SELECT IF4ORDEN.F4NUMORD FROM ((IF4ORDEN INNER JOIN REGISDOC ON IF4ORDEN.F4NUMORD = REGISDOC.F4OCOMPRA) INNER JOIN PAG_DCTO ON REGISDOC.F4CORRELA = PAG_DCTO.correla) INNER JOIN EF2PROVEEDORES ON IF4ORDEN.F4CODPRV = EF2PROVEEDORES.F2NEWRUC WHERE ((PAG_DCTO.NRO_COMP='" & dxDBGrid1.Columns.ColumnByFieldName("F4NUMORD").Value & "') AND (([if4orden].[f4monto]-[regisdoc].[f4total])<=-0.02 Or ([if4orden].[f4monto]-[regisdoc].[f4total])>=0.02)) AND ((PAG_DCTO.CORRELA)=" & Grid.Columns(0).Value & ")"
'    Set rsrep = Af.OpenSQLForwardOnly(Cad, cconex_dbbancos)
