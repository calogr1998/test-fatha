...
Begin VB.Form aprobacion 
   Caption         =   "Acceso a Firma de Aprobación"
   ClientHeight    =   2655
   ClientLeft      =   3855
   ClientTop       =   2850
   ClientWidth     =   4125
...
Static intento As Byte
    ...
    Select Case Index
        Case 0
            ...
            'Verifica Password
            contraseña = numero()
            If Val(txtpassword.Text) <> contraseña Then
                If intento = 3 Then GoTo SinAcceso
            ...
            If rstaprob.EOF Then
                If intento = 3 Then GoTo SinAcceso
                MsgBox "Ud. no tiene autorización para aprobar Pedidos.", vbExclamation, "Sistema de Logística"
                txtusuario.SetFocus
                Exit Sub
            End If
            ...
            If rstaprob.EOF Then
                If intento = 3 Then GoTo SinAcceso
                MsgBox "Ud. no puede aprobar pedidos de este Centro de Costo", vbExclamation, "Sistema de Logística"
                txtusuario.SetFocus
                Exit Sub
            End If
            
            resp = MsgBox("Aprobará la Solicitud." & Chr$(13) & "¿Está Seguro?", vbQuestion + vbDefaultButton1 + vbYesNo, "Sistema de Logística")
            If resp = vbYes Then
