VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} acr_movoc2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Movimiento de Ordenes de Compra"
   ClientHeight    =   11175
   ClientLeft      =   1395
   ClientTop       =   330
   ClientWidth     =   17190
   ...
   _ExtentX        =   30321
   _ExtentY        =   22199
   SectionData     =   "acr_movoc2.dsx":0000
End
Attribute VB_Name = "acr_movoc2"
...
Private Sub GroupFooter1_BeforePrint()
  ...
  '  If Val(Format(flddiferk.Text, "0.00")) < 0 Then
   ...
   ' End If
    ...
    '''''''''''''''''''''''''''''''''Field35.Text = Format(Field11.Text - Field33.Text, "#,##0.00")
    'SALIDAK_SUB.ForeColor = &H0&
        'SALIDAK_SUB.Text = Format(SALIDAK_SUB, "#,##0.00")
    'End If
    'subtotal1(1) = subtotal1(1) + CDbl(ENTRADAK_SUB.Text)
    'saldopprovisionados.Text = Format(100 - Format(CDbl(Field32.Text), "#,##0.00"), "#,##0.00")
    flddiferk.Text = Format(100 - Format(CDbl(Field27.Text), "#,##0.00"), "#,##0.00")
   ' subtotal1 = subtotal1 + CDbl(SALIDAK_SUB.Text)
    ...
    Field26.Text = Format(Field11.Text - Field24.Text, "#,##0.00")
End Sub
